/*
	...
	Bahasa	: NodeJS
	Modul	: express, ws, opencv4nodejs, gm
	...
	Algorithm 	:
		...
		4. HOGDescriptor detectMultiscaleROI	:
...
module.exports = function(){
	try{
		...
		var mser = new ocv.MSERDetector(delta=5, minArea=100, maxArea=1000, maxVariation=0.25, minDiversity=0.2, maxEvolution=100, areaThreshold=1.01, minMargin=0.003, edgeBlurSize=5);
		...
		setInterval(function(){
			...
			gm(ocv.imencode('.jpg', _cam)).crop(_cam.sizes[1] - (_cam.sizes[1] * opt.zoom[0]), _cam.sizes[0] - (_cam.sizes[0] * opt.zoom[0]), _cam.sizes[1] * opt.zoom[1], _cam.sizes[0] * opt.zoom[2]).toBuffer(function(e,b){
				...
				var dt_mser = mser.detectRegions(frm2);
				...
				for(var i in dt){
					...
					if(dt[i].y <= frm2.sizes[0] - (frm2.sizes[0]*opt.line) && dt[i].y + dt[i].height >= frm2.sizes[0] - (frm2.sizes[0]*opt.line)){
						det.push(dt[i]);
						c = new ocv.Vec(0,0,255);
					}
